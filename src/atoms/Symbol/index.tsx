...
const Symbol = (props: SymbolProps): ReactElement => {
  const type = props.type || 'cost';
  const appearanceClass = type == 'cost' ? styles['ms-cost'] : styles[props.color || 'text'];
  const manaFontClassNames = [styles.ms, appearanceClass, styles[getClassName(props.value)]];
  return (
    <i className={[styles.mana, styles[props.size || 'medium'], ...manaFontClassNames].join(' ')}></i>
  );
}
...
const getClassName = (value: SymbolString): string => {
  switch(value) {
    case 'W':
      return 'ms-w';
    case 'B':
